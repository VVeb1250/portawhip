...
3. The same workflow detects the fresh tag (`release_created` output) and runs the `publish` job: full release gates (`npm test`, `npm run route:eval`, `npm run doctor`, `npm audit --audit-level=high`) then `npm publish --provenance`.
